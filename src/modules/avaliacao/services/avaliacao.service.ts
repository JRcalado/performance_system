...
import { HabilidadesService } from '../../habilidades/services/habilidades.service';
...
export class AvaliacaoService {
  ...
  async calcularPercentualPorPilar(
    ...
    const avaliacoes = await this.calcularPesos(colaboradorId);
    const habilidadesPorPilar = avaliacoes.filter(
      (habilidade) => habilidade.competencia.pilar.id === pilarId,
    );

    const totalHabilidades = habilidadesPorPilar.length;
    const habilidadesAtingidas = habilidadesPorPilar.filter(
      (h) => h.peso === 1,
    ).length;
  ...
  async calcularPercentualPorCompetencia(
    ...
    const avaliacoes = await this.calcularPesos(colaboradorId);
    const habilidadesPorCompetencia = avaliacoes.filter(
      (habilidade) => habilidade.competencia.id === competenciaId,
    );

    const totalHabilidades = habilidadesPorCompetencia.length;
    const habilidadesAtingidas = habilidadesPorCompetencia.filter(
      (h) => h.peso === 1,
    ).length;
    ...
    return {
      competenciaId,
      percentual,
      habilidades: habilidadesPorCompetencia,
    };
  }
}
